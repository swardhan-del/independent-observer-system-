...
| Publication catalogue | Volume filters and local query exposed volume records only | Each volume now carries its matched SSRN reading copies, paper descriptions/categories/statuses are indexed by the local query, and every paper offers both an internal reading-copy link and a direct SSRN record link; the hero now states plainly that the four lines of inquiry are the four volumes; retrieved downloads remain usage signals, not ratings |
...
Verification on 2026-08-26: format check passed; Astro check passed with 0 errors, 0 warnings, and 0 hints; `npm test` passed with 170 tests across 9 files; production and GitHub Pages builds each produced 38 static pages; built-output tests passed with 170 tests; the custom-domain root build passed; the SEO audit passed all 38 HTML files; and the public artifact leakage scan passed. A green result does not authorize merge, article publication, or Dropbox mutation.
