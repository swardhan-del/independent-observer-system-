...
| Discovery          | Home cards, six coverage fields, four-volume roadmap   | `/start/`, six topic hubs, a Topics-page four-volume research spine linking each volume to its subject lenses, honest preview/release labels, related-content model, and roadmap timeline                                                                             |
| Public library     | Three high-level volume summaries and a document shelf | Four public-safe volume content blocks with core ideas, topic lenses, roadmap links, local URL filtering, and seven SSRN preprint cards linked to their internal readers and public SSRN records                                                                      |
...
- Topic navigation: the Topics page now leads with all four roadmap volumes, each connected to at least two subject hubs and its existing series detail route; no topic card is presented as a released article.
- Library navigation: the Public Library now leads with four volume blocks, each showing core ideas, topic lenses, roadmap links, and mapped SSRN preprint reading copies.
...
Verification on 2026-08-26: format check passed; Astro check passed with 0 errors, 0 warnings, and 0 hints; `npm test` passed with 161 tests across 9 files; production and GitHub Pages builds each produced 38 static pages; built-output tests passed with 161 tests; the custom-domain root build passed; the SEO audit passed all 38 HTML files; and the public artifact leakage scan passed. A green result does not authorize merge, article publication, or Dropbox mutation.
