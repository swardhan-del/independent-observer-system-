...
const allowedKinds = new Set(["research", "documentary"]);
const excludedTopicPattern =
  /\b(?:medical(?:[-\s]+school)?|med[-\s]+school|medicine|personal|private)\b/i;
const documentExtensions = new Set([".docx", ".pdf", ".pptx", ".md", ".txt"]);
const documentaryExtensions = new Set([".m4v", ".mov", ".mp4", ".webm"]);
const maxItems = 100;
...
function assertReleaseGates(manifest) {
  ...
  if (
    ...
    requiredReleaseGates.some((gate) => manifest.releaseGates[gate] !== true)
  ) {
    throw new Error(
      "The Dropbox manifest must set every release gate to true: " +
        requiredReleaseGates.join(", ") +
        ".",
    );
  }
}

function validateSource(source, kind, prefix) {
  if (!source || typeof source !== "object") {
    throw new Error(prefix + ".source is required.");
  }
  const relativePath = cleanText(source.relativePath, prefix + ".source.relativePath", 300);
  if (
    relativePath.startsWith("/") ||
    relativePath.includes("\\") ||
    relativePath.split("/").some((part) => part === "" || part === "." || part === "..")
  ) {
    throw new Error(prefix + ".source.relativePath must stay below the approved Dropbox folder.");
  }
  const sha256 = cleanText(source.sha256, prefix + ".source.sha256", 64).toLowerCase();
  if (!/^[a-f0-9]{64}$/.test(sha256)) {
    throw new Error(prefix + ".source.sha256 must be a 64-character lowercase SHA-256 digest.");
  }
  if (source.qualityChecked !== true) {
    throw new Error(prefix + ".source.qualityChecked must be true.");
  }
  const extension = extname(relativePath).toLowerCase();
  const allowedExtensions = kind === "research" ? documentExtensions : documentaryExtensions;
  if (!allowedExtensions.has(extension)) {
    throw new Error(
      prefix +
        ".source.relativePath has an unsupported extension for " +
        kind +
        ": " +
        extension,
    );
...
function validateManifest(manifest) {
  if (manifest?.schemaVersion !== 2) {
    throw new Error("The Dropbox manifest must use schemaVersion 2.");
  }
  assertReleaseGates(manifest);
  ...
  return manifest.items
    .map((item, index) => {
      ...
      const result = {
        ...
        status: item.status,
        source: validateSource(item.source, item.kind, prefix),
      };
      ...
      if (item.readingTime !== undefined) {
        ...
        assertPublicTopic(result.readingTime, `${prefix}.readingTime`);
      }
      return result;
    })
...
function render(items) {
  ...
  const serialized = JSON.stringify(publicItems, null, 2);
